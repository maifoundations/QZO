...
2. Create a CONDA environment and install the required packages
   ...
   Note that `gptqmodel==1.7.2` with `Triton` inference kernel is required to reproduce the results with GPTQ. Otherwise, unexpected behaviour will be observed, such as divergence when using the Marlin kernel (see this [issue](https://github.com/maifoundations/QZO/issues/4) for more details). 
